 ...
 */
DBusHandlerResult Cloud_Push_DbusStub::server_get_all_properties_handler(DBusConnection *conn, DBusMessage *reply)
{
...
DBusHandlerResult Cloud_Push_DbusStub::server_message_handler(DBusConnection *conn, DBusMessage *message, void *data)
{
	...
	} else if (dbus_message_is_method_call(message, "org.cloudPush.ServerInterface", "EmitSignal")) {

		if (!(reply = dbus_message_new_signal("/org/cloudPush/ServerObject",
						      "org.cloudPush.ServerInterface",
						      "OnEmitSignal")))
			goto fail;
...
int Cloud_Push_DbusStub::Cloud_Push_DbusStubInit(void)
{
	DBusConnection *conn;
	DBusError err;
	int rv;
	...
	rv = dbus_bus_request_name(conn, "org.cloudPush.Server", DBUS_NAME_FLAG_REPLACE_EXISTING , &err);
...
int main()
{
	Cloud_Push_DbusStub obj;
	obj.Cloud_Push_DbusStubInit();
}
